...
{
	virtual void OutputSingles(std::string sequenceName, size_t x, size_t xSeq, StringSlice repeat) = 0;
	virtual void OutputPairs(std::string sequenceName, size_t x, size_t y, size_t xSeq, size_t ySeq, StringSlice repeat, StringSlice complement) = 0;
	virtual void AfterEverySequence(std::vector<size_t> repeatCount) = 0;
	virtual void Initialize(const CommandLine&) = 0;
	virtual bool InitializeForEverySequence(const std::string sequenceName, const std::vector<std::string>& containedSequences, const int version, const CommandLine& c) = 0;
	virtual void BeforeAmbiguousRepeats()=0;
...
public:
	...
	void OutputSingles(std::string sequenceName, size_t x, size_t xSeq, StringSlice repeat);

	void OutputPairs(std::string sequenceName, size_t x, size_t y, size_t xSeq, size_t ySeq, StringSlice repeat, StringSlice complement) override;

	void AfterEverySequence(std::vector<size_t> repeatCount);

	void BeforeAmbiguousRepeats();

	virtual void Initialize(const CommandLine& c);

	bool InitializeForEverySequence(const std::string sequenceName, const std::vector<std::string>& containedSequences, const int version, const CommandLine& c) override;
};
...
public:
	...
	void Initialize(const CommandLine& c);
