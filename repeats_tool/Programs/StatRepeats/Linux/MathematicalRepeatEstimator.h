#ifndef MATHEMATICAL_REPEAT_ESTIMATOR_H
#define MATHEMATICAL_REPEAT_ESTIMATOR_H
...
class MathematicalRepeatEstimator
{   
	size_t _seqLen;
	size_t _fragLen;
	size_t _fragCount;
	...
	double ForRepeats(size_t searchStart, size_t last, int step);

	const Factors* _factors;

public:
	MathematicalRepeatEstimator(size_t seqLen, size_t fragLen, size_t alphabetSize, const Factors& factors);
	double Compute(size_t num);
};
