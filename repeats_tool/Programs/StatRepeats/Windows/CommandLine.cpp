...
CommandLine::CommandLine(int argsLength, char** args) :
	fragmentLength_(0), printInstances_(false), probability_(false), isMathematical_(false), isRepeat_(false), split_(false),
	logging_(false), proteins_(false), rna_(false), dna_(false), commitCount_(0), maxGap_(0), pval_(0), computeStatistics_(false),
	exclude_(0), alphabetReduction_(0), mapping_ () {
	...
	auto parsed = ParseCommandLine(
		argsLength, args,
		{ "input", "length" },
		{ "printinstances", "maxgap", "type", "embeddedsql", "pvalue", "exclude", "letter", "load", "commitcount", 
	...
	if (parsed["help"] != "true") {
		...
		excludeLetter_ = ReadCommandLineArgument(parsed, "letter", string(), stringToString);
		if (exclude_ != -1 && excludeLetter_ == "") excludeLetter_ = "N";
		...
		//if (!combinedSequenceName_.empty()) probability_ = false;
